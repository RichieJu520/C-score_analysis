...
for (n in T_list){
  ...
  csModel <-cooc_null_model(OTU_Abu, algo = "sim9", metric = "c_score_var",
                            nReps = 30000, saveSeed = FALSE, burn_in = 500, algoOpts = list(),
                            metricOpts = list(), suppressProg = FALSE);
  n
  summary(csModel);
  
  write.table(n,"c-score.var.N1W1.xls",append = TRUE);
  sink("c-score.var.N1W1.xls", append = TRUE);
  summary(csModel);
  sink(NULL);
  
  emf(file = sprintf("%s.c-score.var.hist.30000.emf",n), width = 7, height = 7,
      bg = "transparent", fg = "black", pointsize = 12,
      family = "Helvetica", custom.lty = FALSE);
  plot(csModel,type = "hist");
  dev.off();
  
  emf(file = sprintf("%s.c-score.var.burnin.30000.emf",n), width = 2.1, height = 2.1,
      bg = "transparent", fg = "black", pointsize = 12,
      family = "Helvetica", custom.lty = FALSE);
